    ...
    for j in range(len(numsToBeDrawn)):
        for player_board in player_boards:
            ...
            if check_if_row_or_col(player_board): # check if winner
                return [calledNums, player_board]
...
winningCall = mark_nums(numsToBeDrawn, player_boards)[0][-1] # last call
winning_board = mark_nums(numsToBeDrawn, player_boards)[1] # winning board
print(calculate_winning_score(winning_board) * winningCall)
print(winning_board)