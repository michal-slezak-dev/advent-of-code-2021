...
for points in coordinates: # points[0] -> x1 ; points[1] -> y1 ; points[2] -> x2 ; points[3] -> y2
    ...
    if points[0] != points[2] and points[1] != points[3] and points[0] < points[2] and points[0] == points[1] and points[2] == points[3]: # check if x1 != x2 & y1 != y2 & x1 = y1 & x2 and those points are like 1,1 -> 3,3

        for i in range(points[0] + 1, points[2]):
            floatPoint = f"{i}.{i}"
            all_lines.append(floatPoint)

    elif points[0] != points[2] and points[1] != points[3] and points[0] > points[2]: # check if x1 != x2 and y1 != y2 and those points are like 1,1 -> 3,3
        
        y = 1 # it's responsible for increasing the y coord. x,y => 8,0 -> 7,1 -> 6,2 ...
        i = points[0] - 1 # it's responsible for decreasing the x coord. x,y => 8,0 -> 7,1 -> 6,2 -> ...
        while i > points[2]:
            floatPoint = f"{i}.{points[2] + y}"
            all_lines.append(floatPoint)
            y += 1
            i -= 1

    elif points[0] == points[2] and points[1] < points[3]: # check if x1 = x2 and those points are like: 1,3 -> 1,5
...
pprint(len(num_of_occurances)) # display the num of points that overlap (diagonal as well)